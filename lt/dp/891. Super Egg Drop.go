...
计算方法：我们在计算第N层的时候，尝试从每一层x[1<=x<N]把鸡蛋扔下来，分别看碎和不碎两种情况需要的最少测试次数的最大值【因为要cover最坏的情况，才能说我解决了从第x层扔下的问题】。 然后取最小值，因为我们使用最优的方案，将穷举出来的各个层扔下的最坏情况都得到，取最小值，也就是需要知道从哪一层扔下来的方法是需要测试次数最少的。
有点极大极小值算法MinMax的意思
*/
