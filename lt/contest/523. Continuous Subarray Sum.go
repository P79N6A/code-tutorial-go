...
func checkSubarraySum(nums []int, k int) bool {
    ...
    sum,num := 0,0
    return selectarr(nums,0,k,&sum,&num)
}
func selectarr(nums []int,start,k int,sum,num *int) bool {
    if start >= len(nums) {
        if *sum%k == 0 && *num > 1{
            return true
        }
    ...
    *sum += nums[start]
    *num += 1
    r := selectarr(nums,start+1,k,sum,num)
    if r {
        return true
    }
    *sum -= nums[start]
...
func main() {
    fmt.Println(checkSubarraySum([]int{0,1,0},0))
